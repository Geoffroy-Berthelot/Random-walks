...
The walker walks throught edgesat speed $v$ and visits nodes. At each new (unvisited) node/site the walker pauses for duration $c$, which can be set to 0.
The distance $y$ between two nodes is defined as $y = L / ( X^(1/d) -1 )$ where $L$ is the size of the graph, and $X$ the 'density' (or number of nodes in the graph at starts).
